...
class DataPortal(object):
    ...
    def _get_fetcher_value(self, asset, field, dt):
        ...
        try:
            return \
                self._augmented_sources_map[field][asset].loc[session, field]
        except KeyError:
            return np.NaN
    ...
    def _check_is_currently_alive(self, asset, dt):
        ...
        if self._asset_start_dates[sid] > dt:
            raise NoTradeDataAvailableTooEarly(
                sid=sid,
                dt=self.trading_calendar.minute_to_session_label(dt),
                start_dt=start_date
            )
    ...
    def get_fetcher_assets(self, dt):
        ...
        if session_label in self._extra_source_df.index:
            assets = self._extra_source_df.loc[session_label]['sid']
        else:
            return []
