...
METADATA_HEADERS = ['start_date', 'end_date', 'auto_close_date', 'symbol', 'exchange', 'asset_name']
UNWANTED_EXCHANGES = set(["OTC", "OTCBB", "INDX"])


def from_zacks_dump(file_name, start=None, end=None):
    """
    Load data from a Zacks dump from Quandl, the data dump is assumed to be in a .csv file located at: file_name.

    https://www.quandl.com/databases/ZEP
    https://www.quandl.com/databases/ZEP/documentation/api
    Data goes back to 1984 (with only close prices), close and volume starting in 1990.
    Full OHLCV starts on 1999-04-05, an example line
    ITL,INTC,INTEL CORP,,NSDQ,USD,1999-04-14,30.688,30.75,28.125,28.5,104503200.0

    Tickers come from one of the following exchanges: 'NYSE', 'NSDQ', 'OTC', 'ARCA', 'INDX', 'TSXV', 'OTCBB', 'TSX'
    This can take a long time on the order of hours.  Using the defaults on a modern laptop will take 1-2 hours.
    ...
    register("ZacksQuandl", from_zacks_dump("/path/to/your/Zacks/dump/ZACKS_P_f53fjdkj993857.csv"),)
    ...
    def ingest(environ,
        ...
        df = df.drop(['comp_name_2', 'currency_code'], axis=1)  # drop unused columns
        # using inplace=true here instead of functional approach saves peak memory usage

        df = df.ix[START_DATE:]        # drop dates before START_DATE
        gc.collect()  # force garbage collection to free up memory
        df = df.dropna()               # drop row with NaNs or the loader will turn all columns to NaNs

        uv = df.m_ticker.unique()      # get unique m_tickers (Zacks primary key)

        sec_counter = 0  # counter of valid securites, this will be our primary key
        data_list = []                 # list to send to daily_bar_writer
        metadata_list = []             # list to send to asset_db_writer (metadata)

        # iterate over all the unique securities and pack data, and metadata for writing
        for tkr in uv:
            ...
            print " preparing {} / {} ".format(row0["ticker"], row0["exchange"])

            # update metadata
            # 'start_date', 'end_date', 'auto_close_date', 'symbol', 'exchange', 'asset_name'
            ...
            df_tkr = df_tkr.drop(['m_ticker', 'ticker', 'comp_name', 'exchange'], axis=1)
        ...
        print "a total of {} securities were loaded into this bundle".format(sec_counter)
