...
class TradingCalendar(object):
    ...
    def __init__(self, start, end, timezone):
        ...
        if not end:
            ...
            # Give an aggressive buffer for logic that needs to use the next
            # trading day or minute.
            end = end_base + timedelta(days=365)

        self.end = end

        self.non_trading_days = self.get_non_trading_days(start, end)

        self.non_trading_days_offset = \
            pd.tseries.offsets.CDay(holidays=self.non_trading_days)
        self.trading_days = \
            pd.date_range(start=start.date(), end=end.date(),
                          freq=self.non_trading_days_offset).tz_localize('UTC')

        self.early_closes = self.get_early_closes(start, end)
        self.open_and_closes = self.get_open_and_closes(self.trading_days,
                                                        self.early_closes)

    def canonicalize_datetime(self, dt):
    ...
    def get_non_trading_days(self, start, end):
        ...
        non_trading_ruleset = self.get_non_trading_ruleset(start, end)
        non_trading_days = non_trading_ruleset.between(start, end, inc=True)
        non_trading_days += self.get_exceptional_non_trading_days()
        non_trading_days.sort()
        return pd.DatetimeIndex(non_trading_days)

    def get_non_trading_ruleset(self, start, end):
        """Get a ruleset with all the non trading days for this calendar"""
        ...
        non_trading_ruleset.rrule(weekends)
    ...
    def get_early_closes(self):
        return []

    def get_exceptional_open_and_close(day):
        raise RuntimeError(
            "no exceptional open and close time defined for day: "
            + repr(day))

    def get_default_open_close_times(self):
        return self.OPEN_CLOSE_TIMES

    def get_open_and_close(self, day):
        times = self.get_exceptional_open_and_close_times(day) \
            if day in self.early_closes \
            else self.get_default_open_close_times()

        open_datetime, close_datetime = map(partial(operator.add, day), times)

        market_open = \
            pd.Timestamp(open_datetime, tz=self.timezone).tz_convert('UTC')
        market_close = \
            pd.Timestamp(close_datetime, tz=self.timezone).tz_convert('UTC')

        return market_open, market_close

    def get_open_and_closes(self, trading_days, early_closes):
        open_and_closes = pd.DataFrame(index=trading_days,
                                       columns=('market_open', 'market_close'))

        open_and_closes['market_open'], open_and_closes['market_close'] = \
            zip(*open_and_closes.index.map(self.get_open_and_close))

        return open_and_closes


class NyseTradingCalendar(TradingCalendar):
    """NYSE trading calendar with all USA holidays and exceptional non trading
    days."""
    ...
    def __init__(self, start=pd.Timestamp('1990-01-01', tz='UTC'),
                 end=None):
        super(NyseTradingCalendar, self).__init__(start, end, 'US/Eastern')
    ...
    def get_exceptional_non_trading_days(self):
        ...
        # 9/11/2001. It did not open again until 9/17/2001.
        #
        #    September 2001
        ...
        exceptional_days.append(datetime(1994, 4, 27, tzinfo=pytz.utc))
        # - President Ronald W. Reagan - June 11, 2004
        exceptional_days.append(datetime(2004, 6, 11, tzinfo=pytz.utc))
        # - President Gerald R. Ford - Jan 2, 2007
        exceptional_days.append(datetime(2007, 1, 2, tzinfo=pytz.utc))
    ...
    def get_early_closes(self, start, end):
        ...
        day_after_thanksgiving = rrule.rrule(
            rrule.MONTHLY,
            bymonth=11,
            # 4th Friday isn't correct if month starts on Friday, so restrict
            # to day range:
            byweekday=(rrule.FR),
